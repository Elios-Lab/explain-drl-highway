...
class Evaluation(object):
    ...
    def test(self):
        ...
        try:
            ...
            pass
        # state, action, img, q_val = self.run_episodes()
        self.run_episodes()
        self.close()
        # return state, action, img, q_val
    ...
    def run_episodes(self):
        for self.episode in range(self.num_episodes):
            ...
            while not terminal:
                ...
                reward, terminal, previous_observation, action, img, qval = self.step()
                rewards.append(reward)
                img_log.append(img)
                qval_log.append(qval)
            ...
            if (self.training == False):
                ...
                self.explain_dict['img'] = img_log
                self.explain_dict['qval'] = qval_log
            #   return state_log, action_log, img_log, qval_log
    ...
    def step(self):
        ...
        actions = self.agent.plan(self.observation[0])
        if not actions:
        ...
        qval = self.agent.get_state_action_values(previous_observation)
        qval = np.exp(qval)/sum(np.exp(qval)) # softmax

        # Record the experience.
        try:
            self.agent.record(previous_observation, action, reward, self.observation[0], terminal, info)
        except NotImplementedError:
            pass

        return reward, terminal, previous_observation, action, img, qval
